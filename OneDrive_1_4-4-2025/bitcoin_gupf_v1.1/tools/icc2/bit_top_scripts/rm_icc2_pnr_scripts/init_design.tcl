...
START_TIMER bt_timer "Starting Bit Top ICC2 Init"
	...
	source -e import_from_dp.tcl
        ...
        if {$DESIGN_STYLE == "flat"} {
                ...
                if {$PHYSICAL_HIERARCHY_LEVEL == "bottom"} {
                	open_lib ${DESIGN_NAME}.nlib
                        set wd [pwd]
                  #      set_ref_libs -add $wd/bit_slice.nlib
                	open_block ${DESIGN_NAME}/init_design
                        echo "END OPENING BLOCK"
			...
			source -e create_softlinks_to_subblocks.tcl
                	open_lib ${DESIGN_NAME}.nlib
                        #echo "END OPENING LIB"
                        #set wd [pwd]
                        #echo "START ADDING REFERENCE"
                        #set_ref_libs -add $wd/bit_slice.nlib
                        #echo "START OPENING BLOCK"
                	open_block ${DESIGN_NAME}/init_design
                        link -f -rebind
			...
			## For top or intermediate level of hier designs, associate the UPF strategies to the power management cells 
			if {$DESIGN_STYLE == "hier" && $USE_ABSTRACTS_FOR_BLOCKS != ""} {
    				associate_mv_cells
			}
                }
...
#set_ref_libs -remove ./bit_slice.nlib
#set_ref_libs -add ../bit_slice/bit_slice.nlib
#link -f -rebind
...
source -e -v ./scripts/preroute_bt.tcl
...
STOP_TIMER bt_timer "Completing Bit Top ICC2 Init"
exit 
